...
const FigureCard: React.FC<FigureCardProps> = ({ figure, parentLesson, thumbnailUrl, videoUrl, figureCategories, schools, instructors, onRefresh, itemIds, baseRoute, onForceDelete }) => {
  ...
  const shouldPlay = ((settings.autoplayGalleryVideos && isVisible) || isHovering) && !!videoUrl && !!parentLesson;
  useVideoPlayback({ videoRef, item: figure, shouldPlay, onVideoVisibilityChange: setIsVideoVisible });

  // Effect to clear errors when figure changes
  useEffect(() => {
    setError(null);
  }, [figure.id]);
  ...
  return (
    <>
      <div
          ...
          role="button"
          tabIndex={0}
          data-component="figure-card"
          data-item-id={figure.id}
